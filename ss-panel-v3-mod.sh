...
install_ss_panel_mod_v3(){
	...
	num=$1
	if [ "${num}" != "1" ]; then
  	  wget -c --no-check-certificate https://raw.githubusercontent.com/ruike007/feiyan-ss-panel-and-ss-py-mu/master/lnmp1.4.zip && unzip lnmp1.4.zip && rm -rf lnmp1.4.zip && cd lnmp1.4 && chmod +x install.sh && ./install.sh lnmp
	fi
	cd /home/wwwroot/
...
Libtest(){
	...
	if [ "$libAddr" == "$GIT" ];then
		libAddr='https://raw.githubusercontent.com/ruike007/feiyan-ss-panel-and-ss-py-mu/master/libsodium-1.0.13.tar.gz'
	else
...
install_centos_ssr(){
	...
	ldconfig
	git clone -b manyuser https://github.com/ruike007/node_shadowsocks.git "/root/shadowsocks"
	cd /root/shadowsocks
...
install_node(){
	...
	wget -N -P  /etc/ --no-check-certificate  https://raw.githubusercontent.com/ruike007/feiyan-ss-panel-and-ss-py-mu/master/supervisord.conf
	supervisord
	#iptables
	iptables -F
	iptables -X  
...
install_panel_and_node(){
	...
	sed -i '$a * hard nofile 512000\n* soft nofile 512000' /etc/security/limits.conf
	install_centos_ssr
	wget -N -P  /root/shadowsocks/ --no-check-certificate  https://raw.githubusercontent.com/ruike007/feiyan-ss-panel-and-ss-py-mu/master/userapiconfig.py
	# 启用supervisord
